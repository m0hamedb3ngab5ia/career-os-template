...
| Rule | Default | Where |
|---|---|---|
| Cap: submitted in the window + queued/prepared/needs_review | 2 per 90 days | `volume.max_per_company_per_90_days`; per company: `company_caps` in `config/companies.yaml` |
...
opens. `careeros jobs list --order urgent` puts jobs that must go now first. On an interview or assessment,
